...
public interface OutboxEventRepository extends ReactiveCrudRepository<OutboxEvent, UUID> {
    ...
    @Query("UPDATE quiz.outbox_events SET status = 'PROCESSED', processed_at = NOW() WHERE id = CAST(:id AS uuid) AND status = 'NEW'")
    Mono<Void> markProcessed(UUID id);

    // Added WHERE status = 'NEW' to ensure we only update events that are still NEW
    @Query("UPDATE quiz.outbox_events SET status = 'FAILED', error_message = :errorMessage, retry_count = retry_count + 1 WHERE id = CAST(:id AS uuid) AND status = 'NEW'")
