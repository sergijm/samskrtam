...
public class SessionHistoryService {
    ...
    public Flux<SessionHistoryResponse> getSessionHistory(UUID userId, UUID quizId, String quizType, int page, int size) {
        ...
        if (quizId != null && quizType != null) {
            historyFlux = sessionHistoryRepository.findByUserIdAndQuizIdAndQuizType(userId, quizId, QuizType.valueOf(quizType), pageRequest);
        } else if (quizId != null) {
            historyFlux = sessionHistoryRepository.findByUserIdAndQuizId(userId, quizId, pageRequest);
        } else if (quizType != null) {
            historyFlux = sessionHistoryRepository.findByUserIdAndQuizType(userId, QuizType.valueOf(quizType), pageRequest);
        } else {
