...
public class LlmToolSchemaBuilder {
    ...
    /**
     * Возвращает ObjectNode JSON Schema для FunctionDefinition submit_verse_analyses_step1
     * (ШАГ 1: translation + external sandhi + lexical/morphological). Отличается от
     * {@link #buildBatchFunctionDefinitionSchema()} отсутствием formationRuleNumbers
     * (внутренние сандхи, правила 1–40 — это ШАГ 2, см. step2-internal-sandhi.md).
     */
    public ObjectNode buildStep1FunctionDefinitionSchema() {
        ObjectNode params = objectMapper.createObjectNode();
        params.put("type", "object");
        ...
        ObjectNode wordItem = verseProps.putObject("words");
        wordItem.put("type", "array");
        wordItem.put("description", "Grammatical analysis of each word (external sandhi undone)");
        ObjectNode wordItemObj = wordItem.putObject("items");
        wordItemObj.put("type", "object");
    ...
    /**
     * Возвращает ObjectNode JSON Schema для FunctionDefinition submit_word_formations
     * (ШАГ 2: внутренние сандхи / словообразование). Корневой объект: единственное
     * свойство {@code words} — плоский массив (без вложенности по стихам), одна запись
     * на каждое слово батча. Каждая запись содержит verseIndex/position (для джойна
     * к записи шага 1), surfaceIast/lemmaIast/root/derivationalBase/derivationalSuffix
     * (эхо входа) и formationRuleNumbers/formationConfidence/formationNotes.
     */
    public ObjectNode buildStep2FunctionDefinitionSchema() {
        ObjectNode params = objectMapper.createObjectNode();
        params.put("type", "object");
        ...
        wordsNode.put("description",
                "Flat array of per-word internal sandhi analyses — one entry per input word, "
                        + "matched by verseIndex + position to the STEP 1 word");
        ...
        wordProps.putObject("verseIndex").put("type", "integer")
                .put("description", "0-based index of the source verse in the input batch (unchanged from input)");
        wordProps.putObject("position").put("type", "integer")
                .put("description", "Position of the word within its verse (unchanged from input)");
        wordProps.putObject("surfaceIast").put("type", "string")
                .put("description", "Surface form in IAST (echoed from input, not re-derived)");
        wordProps.putObject("lemmaIast").put("type", "string");
        wordProps.putObject("root").put("type", "string");
        wordProps.putObject("derivationalBase").put("type", "string");
        wordProps.putObject("derivationalSuffix").put("type", "string");
        wordProps.putObject("formationRuleNumbers").put("type", "array").putObject("items").put("type", "integer")
                .put("description", "Internal sandhi rule numbers (1–40) applied, in order; [] if none");
