...
@Repository
public interface LexicalTopicBindingRepository extends JpaRepository<LexicalTopicBinding, LexicalTopicBindingId> {
    List<LexicalTopicBinding> findByIdLexemeId(UUID lexemeId);

    List<LexicalTopicBinding> findByIdLexicalTopicId(UUID lexicalTopicId);

    boolean existsByIdLexicalTopicIdAndIdLexemeId(UUID lexicalTopicId, UUID lexemeId);
}