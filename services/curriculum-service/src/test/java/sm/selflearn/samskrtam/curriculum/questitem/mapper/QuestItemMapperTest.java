...
class QuestItemMapperTest {
    ...
    void toDto_matching_suppressesCorrectAnswerInDto() {
        QuestItem item = item("MATCHING", "should-not-leak", "[]",
        ...
        assertThat(dto.answerMode()).isEqualTo("MATCHING");
    ...
    void serializedJson_matching_omitsCorrectAnswerField() throws Exception {
        QuestItem item = item("MATCHING", "should-not-leak", "[]", "{\"x\":1}");
    ...
    void toDto_deserializesDistractorsList() {
        QuestItem item = item("SINGLE_CHOICE", "naraḥ", "[\"naram\",\"narena\",\"narāya\"]", "{\"a\":1}");
