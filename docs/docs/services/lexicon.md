...
> [content-service.md §11](./content-service.md#11-internal-rest-приём-словаря-из-sangraha-service),
...
1. **`dictionary-service`** (`d_mw`/`d_fri` схемы) — это read-only зеркало внешних
   ...
   домен со словарями не конкурирует, а может (опционально, не в этой итерации)
   использовать его как один из источников `meaning`/`gender` при наполнении
   (§ pipeline в `lexicon-content-pipeline.md`).

2. **`content-service.VocabularyWord`/`VocabularyCategory`** (§39, §296–300
   `content-service.md`) — это уже существующая, но **связанная контрактом с
   ...
   `VocabularyWordQuiz`, что в `content-service.md` зафиксировано как
   ...
   **Решение:** не переписывать `content-service.VocabularyWord` немедленно —
   ...
   Существующий sangraha→content-service поток
   ...
   из тех же данных, которые сейчас улетают в `content-service` — это отдельная
...
3. **`curriculum-service.Topic`** (см. `curriculum-service.md`, первая версия
   ...
   - объём операционных издержек нового сервиса (Dockerfile, NetworkPolicy,
     ...
     обсуждался при выделении curriculum-service из content-service) для
...
status (VARCHAR 20, NOT NULL, DEFAULT `DRAFT` — `DRAFT`|`AI_ENRICHED`|`APPROVED`|`REJECTED`, см. `lexicon-content-pipeline.md` §3)
...
исчерпывающе генерируется content-service `declension_forms`/`conjugation`
...
относительно content-service). Детали пересчёта — при реализации, вне периметра
