...
> filterScope=ALL_STEMS) · Задача: [task-03-all-stems-quiz.md](../../agents/tasks/task-03-all-stems-quiz.md)
> Status: **DRAFT**
...
Страница автономна и не привязана к пункту бокового меню (статическое дерево курикулума удалено):
доступ — прямой переход по роуту `/lessons/grammar/declensions-all`.
...
- **По гласным основ** — множественный выбор из 7 значений `vowelType`: A_STEM, AA_STEM, I_STEM,
  II_STEM, U_STEM, UU_STEM, R_STEM. Лейблы — локализованные названия гласных основ (например «a/ā»,
  «i/ī», «u/ū», «ṛ» — точную формулировку фиксирует Агент 3 при реализации), без слова «основы, муж.
  род» — для этого фильтра род выбирается отдельным блоком ниже, лейбл должен быть про гласную, не про
  урок целиком.
...
- Страница доступна по роуту `/lessons/grammar/declensions-all`.
