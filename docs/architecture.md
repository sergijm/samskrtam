...
- `quizStatus` (`CREATED`/`EXISTING`) — единственный сигнал, которым curriculum-service может обозначить, нужен ли срез `progressTagSetId=NEW` при первом старте сессии: `CREATED` → `NEW`, `EXISTING`/кэш-хит → без среза (обычный смешанный отбор). Фронтенд стартует сессию сразу по `quizId` через `POST /quiz/vocabulary/sessions/start-or-resume?lessonId={quizId}&progressTagSetId=...`.
