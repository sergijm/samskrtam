...
Для поддержания компактности документации (лимит 350 строк, см. [`samskrtam-agents-spec.md`](./agents/samskrtam-agents-spec.md) §«Поддержание компактности документации»):

- Если markdown-файл спецификации приближается к лимиту 350 строк или тема естественно распадается на подтемы — создаётся файл-индекс с тем же именем на уровень выше + одноимённая подпапка с деталями.
- Примеры применения: `conventions.md` + `conventions/`, `quiz-service.md` + `quiz-service/`.
