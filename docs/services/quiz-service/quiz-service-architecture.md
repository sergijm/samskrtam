...
Единый сервис для прохождения квизов всех типов (склонения, спряжения, лексика). Обрабатывает жизненный цикл сессии: старт, ответы, завершение. События жизненного цикла пишутся в Transactional Outbox (`quiz.outbox_events`); публикация в Kafka отключена, релей читает outbox вхолостую.
...
WebFlux выбран осознанно из-за интенсивного I/O: одновременные обращения к curriculum-service, запись в Postgres. Реактивный pipeline позволяет держать всё в одном неблокирующем потоке.
