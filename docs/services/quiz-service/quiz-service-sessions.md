...
Все методы сервиса строятся как реактивные цепочки Reactor (Mono/Flux) с использованием R2DBC и WebClient. Ключевой принцип — атомарность: запись в БД и запись события Outbox выполняются в рамках одной транзакционной цепочки.
