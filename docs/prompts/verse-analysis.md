...
> Only rules tagged `"applicability": "external"` (41–71) are relevant to
> `sandhiSplits`; internal rules (1–40) are not word-boundary phenomena — they explain
> a word's *own* form (e.g. why `ghnanti` rather than `hananti`) and are only useful
> as background for the `words[]` grammatical analysis, never for citation in
> `sandhiSplits.ruleNumbers`. Backend injects the external-rules subset (41–71) into
> the request context alongside this prompt.
...
3. sandhiSplits — analysis of every sandhi (sound merger) **between words** in the
   verse (word-boundary / external sandhi only). For each junction point (surface) —
   the list of underlying components (components) it is formed from, and ruleNumbers
   — the list of rule numbers from `sangraha-data/emenau-sandhi-rules.json` (only
...
4. words — a word-by-word grammatical analysis of every word in the verse in order of
   ...
   surfaceDevanagari), the dictionary form/lemma (lemmaIast), the stem (stem), the
   root (root, if applicable — otherwise null), the part of speech (pos), and for
